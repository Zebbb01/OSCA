...
async function createSenior(formData: FormData) {
    ...
    const initialCategoryId = seniorData.pwd
        ? (await prisma.seniorCategory.findUnique({ where: { name: 'Special assistance cases' } }))?.id // Assuming this is for PWD
        : (await prisma.seniorCategory.findUnique({ where: { name: 'Regular senior citizens' } }))?.id; // Assuming this is for non-PWD
    ...
    const senior = await prisma.senior.create({
        data: {
            ...
            Applications: {
                create: {
                    // Assuming a default benefit and status for new applications
                    // You might need to adjust this based on your application flow
                    benefit: { connect: { id: 1 } }, // Connect to a default benefit (e.g., ID 1)
                    status: { connect: { id: 1 } }, // Connect to a default status (e.g., ID 1 for 'Pending')
                    category: initialCategoryId ? { connect: { id: initialCategoryId } } : undefined,
                },
            },
        },
        include: {
            Applications: true, // Include applications to return them
        },
    });
...
async function updateSenior(seniorId: number, updatePayload: any) {
    ...
    // Ensure pwd is a boolean from the payload, default to old status if not provided
    const newPwdStatus = typeof updatePayload.pwd === 'boolean' ? updatePayload.pwd : oldPwdStatus;

    // 2. Update the Senior record
    const updatedSenior = await prisma.senior.update({
        where: { id: seniorId },
        data: {
            ...updatePayload, // Spread all incoming update data
            pwd: newPwdStatus, // Explicitly set the new pwd status (even if it didn't change, for clarity)
            updatedAt: new Date(), // Manually update updatedAt if not handled by Prisma
        },
    });

    // 3. Conditionally update associated Applications based on PWD status change
    if (newPwdStatus !== oldPwdStatus) {
        // Fetch category IDs dynamically to make the code more robust
        ...
        if (!regularCategory || !specialCategory) {
            ...
            // Find all applications for this senior that *don't already* have the target category
            // This prevents unnecessary updates if an application is already correctly categorized
            ...
            if (applicationsToModify.length > 0) {
                // Perform a batch update for all relevant applications
    ...
    return updatedSenior; // Return the updated senior record
}
